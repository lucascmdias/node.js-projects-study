...
function getRootDeps(template, context) {
  if (production && template.ay_) {
    return template.ay_;
  }

  attachDepsAndComponentsToTemplate(template, context);

  var deps = template.ay_ = Object.keys(template.az_).map(key => template.az_[key]);
...
function attachDepsAndComponentsToTemplate(target, context) {
  ...
  if (production && target.az_) return;
  ...
  if (meta.tags) {
    meta.tags.forEach(tagPath => {
      ...
      if (tag.az_) Object.assign(deps, tag.az_);
