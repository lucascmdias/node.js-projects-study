...
AsyncValue.prototype = {
   ...
   * available.
   */
  w_: function (callback) {
    // Do we already have data or error?
    if (this.v_) {
      // invoke the callback immediately
      return callback(this.t_, this.s_);
    }

    var callbacks = this.u_ || (this.u_ = []);
  ...
  y_: function (value) {
    ...
    if (value && typeof value.then === "function") {
      ...
      var finalPromise = value.then(function onFulfilled(value) {
        queueMicrotask(asyncValue.y_.bind(asyncValue, value));
      }, function onRejected(err) {
        queueMicrotask(asyncValue.x_.bind(asyncValue, err));
      });
